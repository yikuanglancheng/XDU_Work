...
for fault_size in range(0,17):
    route_success = 0
    all_hop = 0
    for  all_number in range(0,50000):
        for times in range(0,1):
            routing_lable = np.zeros((9,9))
            for i in range(0,9):
                routing_lable[0][i]=62
                routing_lable[i][0]=62
                routing_lable[8][i]=62
                routing_lable[i][8]=62
                
            flag_Dnode = 1
            while flag_Dnode:
                Dnode_row = random.randint(1,6)
                Dnode_col = random.randint(2,7)
            ...
            flag_Snode = 1
            while flag_Snode:
                Snode_row = random.randint(1,7)
                Snode_col = random.randint(1,7)
            ...
            fault_row = np.random.randint(1,8,size=fault_size)
            fault_col = np.random.randint(1,8,size=fault_size)
            ...
            """
            for i in range(1,8):
                for j in range(7,0,-1): #因为目标节点在东北方向上，所有应该从东北角开始考虑不可达节点的生成
                    if (routing_lable[1][7]!=62)and(routing_lable[1][7]!=255)and(routing_lable[1][7]!=125):
                        routing_lable[1][7]=0
    ...
    fault_rate = fault_size/64
    print("Fault rate is "+str(fault_rate)+"\n")
    average_hop = all_hop/all_number
    print("Average hop  is "+str(average_hop)+"\n")
    if all_number==0:
        continue
    else:
